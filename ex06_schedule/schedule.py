...
def create_schedule_string(input_string: str) -> str:
    ...
    for match in re.finditer(r"( +|\n)([0-1]\d|[2][0-3]|\d)(\D)([0-5]\d|\d)( +)([a-zA-Z]+)", input_string):
        ...
        schedule[time].append(match.group(6))
...
------------------""")
    ...
    for time in sorted(schedule, key=lambda x: (x[6], x[0], x[1], x[3], x[4])):
        if int(time.split(":")[0]) == 0:
            table.append(
                f"| {time.replace(' ', '', 1).replace('0', '12', 1)} | {', '.join(sorted(schedule[time])).lower():<{left_line}} |")
        else:
            table.append(f"| {time} | {', '.join(sorted(schedule[time])).lower():<{left_line}} |")
