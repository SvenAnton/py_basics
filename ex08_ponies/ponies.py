...
def decode(line: str) -> str:
    """Decode given text to utf-8 format string type text."""
...
def read(read_file: str) -> list:
    """Read in given file, decodes lines and returns every line as a dictionary. Returns lines as a list."""
    ponies_list = []
    try:
...
def filter_by_location(ponies: list) -> list:
    """Filter out lines, where location is "None'."""
    return [pony for pony in ponies if not (pony["location"] == "None")]


def filter_by_kind(ponies: list, kind: str) -> list:
    """Filter ponies from list by given "kind" value as a string."""
    return [pony for pony in ponies if (pony["kind"] == kind)]


def get_points_for_color(color: str) -> int:
    """Return points by given color nr. First color gives 10 points, 2nd 9 etc. Points under 5 are returend as None."""
    colors = [
...
def add_points(pony: dict) -> dict:
    """Evaluate ponies by location and returns new dictionary with points added."""
    ...
    if (pony or pony.values() or pony.keys()) is None:
        pony["points"] = None
...
def format_line(pony: dict, place: int) -> str:
    """Put pony directory in right format."""
...
def write(input_file: str, kind: str):
    """Write correctly formatted pony-lines into a file."""
    place_table = "PLACE"
    points_table = "POINTS"
    name_table = "NAME"
    kind_table = "KIND"
