    ...
    for i in range(len(message)):
        if message[i].upper() in alphabet.upper():
            if message[i].isupper():
                message = message.replace(message[i], alphabet[renumber(alphabet.upper().find(message[i]) + shift, alphabet)].upper(), 1)
            else:
                message = message.replace(message[i], alphabet[renumber(alphabet.find(message[i]) + shift, alphabet)], 1)
        else:
            message
    return message
